...
GitHub : https://www.github.com/plfj
Gitlab : https://www.gitlab.com/plfj
...
import sys,random,ast,zlib,os,marshal,base64,bz2,lzma,gzip,struct,io,datetime
from pystyle import *
from pathlib import Path
class DepTraiCoGiSai(BaseException):0
...
def superrandom():
    # return "_" + "".join(__import__("random").sample([str(i) for i in range(1, 20)], k=2))
    return ''.join(__import__('random').choices([chr(i) for i in range(1000, 3000)], k=3503))
...
class FeedO:
    def build_anti_pycdc(self):
        antipycdc = '';antipycdc2 = ''
        crt = ["a","b","c","d","e","f"]
        e = {'Z': '1/0,', 'T': 'len+1,', 'N': 'xyz,', 'T2': '"a"+1,', 'I': '[][99],', 'K': '{}[""],', 'M': "__import__('xyz'),", 'V': 'int("a",99),', 'A': '[].__x,', 'F': 'open("ww"),',}
        for k in e:
            e[k] = e[k] * 300
        antipycdc += '\n'.join((f'\ntry:({v})\nexcept:0\nfinally:int(2010-1999)\n' for v in e.values()))
        for g in crt:
            antipycdc2 = f"你器(你器(你器(你器(你器(你器('HELLO'))))))," * 350;antipycdc2 = f"\ntry:xe=[" + antipycdc2 + f"]\nexcept:pass\n"
            FTR = f"""
def 你器(你):
    return 你
{antipycdc2}\nfinally:int(2010-1999)\n"""
...
def antibypass():
    def anti(s: str, kkk=69):
        ...
        mm = ", ".join(fx)
        return f"""((lambda __pymeo__: __pymeo__(*[__dat__('HELLO DECOMPILER',{mm})]))(lambda *__datcute__: ((lambda __wearelove__, __uyencoder__:__wearelove__().join([*map(lambda n: __uyencoder__().format((n ^ 64)), __datcute__)]))(lambda: getattr(''.__class__, '__add__')('__dat__', ''),lambda: "__yeuquynhngan__"))))"""

    def _anti():
        antipycdc = ''
        for i in range(1000):
            antipycdc += f"__uyencoderdeptrai__(__uyencoderdeptrai__(__uyencoderdeptrai__(__uyencoderdeptrai__(__uyencoderdeptrai__(__uyencoderdeptrai__('HELLO')))))),"
        antipycdc = "try:pymeo=[" + antipycdc + "]\nexcept:pass"
        txf = f"""
...
var = fr"""
try:
    class {_memoryerror}(eval("BaseException")):_ : (lambda :"Tao la dat dep trai")();pass
except BaseException as TUOI_LON_DECODE:pass
...
def {_join}({_wt}):
    ...
    return {__exx}
{nuyenbuiltins} = (lambda a: (lambda b: (lambda c: vars(c.modules['builtins']))(b('sys')))(a['__import__']))(vars(globals()['__builtins__']))
...
nhapuser = input(" ENTER YOUR NAME : ")
...
_file = str(Path(_file).expanduser().resolve())
path = __import__('pathlib').Path(_file)
nmp = path.parent
cmp = path.name
if nmp==".":
    _obo_ = "obf-" + str(cmp)
else:
    _obo_ = str(nmp) + "/obf-" + str(cmp)
while True:
    try:
...
antidebug = input(" ANTI DEBUG (MAIN MODE) ? (y/n): ")
anticrack = input(" ANTI CRACK REQUESTS LIB ? (y/n): ")
#method = input(" DO YOU WANT COMPILE? (y/n): ")
method = "y"
#___print(" OBF LOADING... (10-1000secs) ")

__import__("sys").stdout.write(stage(f" OBF LOADING... (10-1000secs) \r"));__import__("sys").stdout.flush()
...
for i in range(int(12)):
    if int(os.stat(_file).st_size) >= 50000:
        cz = 23
        code = f"_ = lambda __ : __import__('zlib').decompress(__[::-1]);exec((_)({zlib.compress(code.encode('utf8'))[::-1]}))"
import os
import pystyle
...
def NGOCUYEN1907():
    globals()["_"]=[[[[[(('pymeo') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000
if str(__import__('sys')) != "{eval("__import__('sys')")}":[[[[[(('pymeo') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000
if str(__import__('sys').exit) != "{eval("__import__('sys').exit")}":[[[[[(('pymeo') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000
if str(__import__('os')) != "{eval("__import__('os')")}":[[[[[(('pymeo') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000
if str(__import__('os').system) != "{eval("__import__('os').system")}":[[[[[(('pymeo') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000
if str(type(open)) != "<class 'builtin_function_or_method'>":[[[[[(('pymeo') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000
if str(eval("open")) != '<built-in function open>': [[[[[(('pymeo') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000


import os, sys, ctypes, inspect, builtins, hashlib, marshal, types
def detect_debugger():
    try:
...
def hash_func(f):
    if not isinstance(f, types.BuiltinFunctionType):
        return hashlib.sha256(marshal.dumps(f.__code__)).hexdigest()
    return "builtin"
ORIGINALS = {{
    "eval": builtins.eval,
    "exec": builtins.exec,
...
HASHES = {{k: hash_func(v) for k,v in ORIGINALS.items()}}
def check_integrity():
    for k, v in ORIGINALS.items():
        if getattr(builtins, k) is not v:
            NGOCUYEN1907()
    return True
check_integrity()

BANNED = ["frida", "pyrasite", "pyinjector", "gdb", "lldb"]
...
def detect_stack_abuse():
    stack = inspect.stack()
    for frame in stack:
        name = frame.filename.lower()
        if any(x in name for x in ["frida", "gdb", "inject", "trace"]):
            NGOCUYEN1907()
detect_stack_abuse()

class GuardImporter:
    def find_spec(self, name, path, target=None):
        if any(x in name.lower() for x in ["frida", "inject", "trace"]):
            NGOCUYEN1907()
        return None

sys.meta_path.insert(0, GuardImporter())
...
try:
    ...
    if _author != ('ALEX', 'https://www.github.com/plfj','https://www.gitlab.com/plfj'):
        raise
    if ';' in j[0]:
        raise
    if ';' in j[1]:
    ...
    if str(j[5])[-2:] != ")\n":
        raise
    if "except BaseException as e:__import__('sys').exit(e)" != str(j[6]):
        raise
    if str(j[6])[-1] == ';':
        raise
    if _pymeomeo_version != ('PREMIUM','User : {nhapuser}','main'):
        raise
    if _obf != ('pymeoV2')[(lambda : 0)()]:
        raise
    if "('pymeoV2')[(lambda : 0)()]" not in j[1]:
        raise
...
if _author != ('ALEX', 'https://www.github.com/plfj','https://www.gitlab.com/plfj'):
    globals()["_"]=[[[[[(('pymeo') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000
if _pymeomeo != (lambda a: (lambda b: (lambda c: vars(c.modules['builtins']))(b('sys')))(a['__import__']))(vars(globals()['__builtins__'])):
    globals()["_"]=[[[[[(('pymeo') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000
"""
code = antica2+code
...
class PYMEOMEOMEWWW(list):
    ...
    def format_pymeo(pymeomeo_, pymeo):
        row = []
        row.append('bo la nguoi dep trai')
        if pymeo.line:
            stripped_line = pymeo.line.strip()
            row.append('cuts di')

            line = pymeo.pmeooowwwww
            orig_line_len = len(line)
            frame_line_len = len(pymeo.line.lstrip())
            stripped_characters = orig_line_len - frame_line_len
            if (
                pymeo.dethuongvacute is not None
                and pymeo.meomeocute is not None
            ):
                start_offset = _byte_offset_to_character_offset(
                    line, pymeo.dethuongvacute)
                _0xFFFFFFFFF = _byte_offset_to_character_offset(
                    line, pymeo.meomeocute)
                meopymeomeo = line[start_offset:_0xFFFFFFFFF]
                anchors = None
                if pymeo.lineno == pymeo.meomeodethuong:
        ...
        if pymeo.locals:
            for name, pymewwww in sorted(pymeo.locals.items()):
                row.append('aa')
        return ''.join(row)
    ...
    def format(pymeomeo_):
        ...
        ln = None
        count = 0
        for pymeo in pymeomeo_:
            formatted_frame = pymeomeo_.format_pymeo(pymeo)
            if formatted_frame is None:
                continue
            if (lf is None or lf != pymeo.filename or
                ll is None or ll != pymeo.lineno or
                ln is None or ln != pymeo.name):
...
if method.upper() != "Y":
    ...
    #__import__('base64').a85encode(__import__('bz2').compress(__import__('lzma').compress(__import__('zlib').compress(__import__('gzip').compress(__import__('marshal').dumps(code.encode('utf8'),9),9),9),2),9))
    code = compile(code.encode("utf8"), "pymeo", "exec", flags=annotations.compiler_flag, dont_inherit=True, optimize=2)
    code = code.replace(
        co_filename="<pymeo>",
        co_firstlineno=999999,
        co_name="<pymeo>",
        co_qualname="<meomeo>")
    code = marshal.dumps(code, 9)
    code = gzip.compress(code, 9)
    code = zlib.compress(code, 9)
    code = lzma.compress(code, 2)
    code = bz2.compress(code, 9)
...
if str(__import__('sys').version[0:4]) != '{str(eval("__import__('sys').version[0:4]"))}':
    ...
    __import__("sys").stdout.write(">> Loading..\\r");__import__("sys").stdout.flush()
    _pymeomeo['exec'](_pymeomeo['__import__']('marshal').loads(_pymeomeo['__import__']('gzip').decompress(_pymeomeo['__import__']('zlib').decompress(_pymeomeo['__import__']('lzma').decompress(_pymeomeo['__import__']('bz2').decompress(_pymeomeo['__import__']('base64').a85decode({code})))))))
"""
#    code = __import__('base64').a85encode(__import__('bz2').compress(__import__('lzma').compress(__import__('zlib').compress(__import__('gzip').compress(__import__('marshal').dumps(compile(code.encode("utf8"),"pymeo","exec",flags=annotations.compiler_flag,dont_inherit=True,optimize=2),9),9),9),2),9))
    code = marshal.dumps(code.encode('utf8'), 9)
    code = gzip.compress(code, 9)
    code = zlib.compress(code, 9)
    code = lzma.compress(code, 2)
    code = bz2.compress(code, 9)
    code = base64.a85encode(code)
    code = f"""#!/usr/bin/env python{str(__import__('sys').version_info.major)+"."+str(__import__('sys').version_info.minor)}
_obf = ('pymeoV2')[(lambda : 0)()]
_author = ('ALEX', 'https://www.github.com/plfj','https://www.gitlab.com/plfj')
_pymeomeo_version = ('PREMIUM','User : {nhapuser}','{vaidai}')
_pymeomeo = (lambda a: (lambda b: (lambda c: vars(c.modules['builtins']))(b('sys')))(a['__import__']))(vars(globals()['__builtins__']))
try:_pymeomeo['exec'](_pymeomeo['__import__']('marshal').loads(_pymeomeo['__import__']('gzip').decompress(_pymeomeo['__import__']('zlib').decompress(_pymeomeo['__import__']('lzma').decompress(_pymeomeo['__import__']('bz2').decompress(_pymeomeo['__import__']('base64').a85decode({code})))))))
except BaseException as e:__import__('sys').exit(e)""".strip()
    lencode = len(code)
#__import__('requests').get(f'https://sever.ngocbansub.com/keyencuyen/tru.php?key={deptrai}')
open(_obo_, "w", encoding="utf8").write(str(code))
#___print("                                       ")
now = round(__import__('time').time() - now, 3)
__builtins__.print("")
print(f" OBFUSCATION COMPLETED SUCCESFULLY IN {now}s")
print(" SAVED IN",_obo_)
